...
class TestLearningLockerSync:
    ...
    def sample_statement(self):
        ...
        return {
            ...
            "actor": {"account": {"name": "test-user@7taps.com"}},
    ...
    @patch("app.sync_learninglocker.httpx.AsyncClient.post")
    async def test_sync_statement_success(
        self, mock_post, mock_auth, sync_service, sample_statement
    ):
    ...
    @patch("app.sync_learninglocker.LearningLockerSync.get_learninglocker_auth")
    @patch("app.sync_learninglocker.httpx.AsyncClient.post")
    async def test_sync_statement_failure(
        self, mock_post, mock_auth, sync_service, sample_statement
    ):
    ...
    @patch(
        "app.sync_learninglocker.LearningLockerSync.sync_statement_to_learninglocker"
    )
    async def test_sync_redis_to_learninglocker(self, mock_sync, sync_service):
        ...
        with patch.object(sync_service.redis_client, "xreadgroup") as mock_read:
            ...
            with patch.object(sync_service.redis_client, "xack") as mock_ack:
                result = await sync_service.sync_redis_to_learninglocker(
                    max_statements=2
                )
    ...
    async def test_data_migration_with_existing_statements(
        self, mock_sync, sync_service
    ):
    ...
    async def test_performance_with_large_dataset(self, mock_sync, sync_service):
        ...
        for i in range(100):
            large_dataset.append(
                (f"msg_{i}", {"data": json.dumps({"id": f"statement-{i}"})})
            )
    ...
    async def test_data_integrity_verification(self, sync_service, sample_statement):
        ...
        with patch(
            ...
            success = await sync_service.sync_statement_to_learninglocker(
                sample_statement
            )
...
class TestLearningLockerAPI:
    ...
    def test_sync_learninglocker_endpoint(self, client):
        ...
        with patch(
            "app.sync_learninglocker.learninglocker_sync.sync_redis_to_learninglocker"
        ) as mock_sync:
    ...
    def test_sync_status_endpoint(self, client):
        ...
        with patch(
            "app.sync_learninglocker.learninglocker_sync.get_sync_status"
        ) as mock_status:
