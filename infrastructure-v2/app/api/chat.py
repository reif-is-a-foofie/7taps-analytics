...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        result = await chat_service.process_chat_message(request.message)
        
        # Save chat history
        await chat_service.save_chat_message(request.message, result["response"])
        
        # Generate simple visualization if needed
        visualization = None
        if result.get("intent") == "analytics_query":
            visualization = "<div class='chart-placeholder'>Analytics visualization would appear here</div>"
