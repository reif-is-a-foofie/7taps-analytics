    ...
    def check_endpoint(self, endpoint: str, expected_status: int = 200) -> bool:
        ...
        try:
            ...
            if response.status_code == expected_status:
                ...
                self.issues.append(f"Endpoint {endpoint} returned {response.status_code}, expected {expected_status}")
                return False
        except Exception as e:
            self.issues.append(f"Endpoint {endpoint} failed: {str(e)}")
            return False
    
    def check_post_endpoint(self, endpoint: str, data: Dict, expected_status: int = 200) -> bool:
        ...
        try:
            ...
            if response.status_code == expected_status:
                ...
                self.issues.append(f"POST endpoint {endpoint} returned {response.status_code}, expected {expected_status}")
                return False
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "modules" in data and "overall_progress" in data:
                    ...
                    self.warnings.append("Orchestrator progress endpoint missing expected fields")
                    return False
            else:
                self.issues.append(f"Orchestrator progress returned {response.status_code}")
                return False
        except Exception as e:
