...
class ETLStreamingProcessor:
    ...
    async def process_xapi_statement(self, statement: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            flattened = {
                ...
                "actor_id": (
                    actor.get("account", {}).get("name")
                    if isinstance(actor, dict)
                    else None
                ),
                "verb_id": verb.get("id") if isinstance(verb, dict) else None,
                "object_id": (
                    object_data.get("id") if isinstance(object_data, dict) else None
                ),
                "object_type": (
    ...
    async def write_to_postgres(self, flattened_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"Successfully wrote statement {flattened_data.get('statement_id')} to Postgres"
            )
    ...
    async def write_batch_to_postgres(self, batch_data: List[Dict[str, Any]]):
        ...
        try:
            ...
            logger.info(
                f"Successfully wrote batch of {len(batch_data)} statements to Postgres"
            )
            ...
            for data in batch_data:
                try:
                    ...
                    logger.error(
                        f"Error normalizing statement {data.get('statement_id')}: {e}"
                    )
                    continue

            logger.info(
                f"Successfully normalized {normalized_count}/{len(batch_data)} statements"
            )
