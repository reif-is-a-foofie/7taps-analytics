...
async def test_etl_streaming() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"ETL streaming test failed: {str(e)}"
        )
...
async def get_etl_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "database": {
                "url": (
                    etl_processor.database_url.split("@")[-1]
                    if etl_processor.database_url
                    else "not_configured"
                )
            },
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get ETL status: {str(e)}"
        )
...
async def test_etl_incremental() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Incremental ETL test failed: {str(e)}"
        )
...
async def get_incremental_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "database": {
                "url": (
                    incremental_processor.database_url.split("@")[-1]
                    if hasattr(incremental_processor, "database_url")
                    and incremental_processor.database_url
                    else "not_configured"
                )
            },
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get incremental ETL status: {str(e)}"
        )
