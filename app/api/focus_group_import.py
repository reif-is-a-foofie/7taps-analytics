...
def convert_focus_group_to_xapi_statement(
    record: Dict[str, Any], cohort_id: str
) -> Dict[str, Any]:
    ...
    statement = {
        ...
        "object": {
            ...
            "objectType": "Activity",
            "definition": {
                "name": {"en-US": card_info.get("question", record["card"])},
                "description": {
        ...
        "result": {
            ...
            "extensions": {
                "https://7taps.com/response-length": (
                    len(record["response"]) if record["response"] else 0
                )
            },
        },
        "context": {
            ...
            "extensions": {
                ...
                "https://7taps.com/lesson-url": LESSON_URL_MAPPING.get(
                    str(record["lesson_number"]), ""
                ),
    ...
    try:
        ...
        for record in focus_group_data:
            try:
                ...
                imported_count += 1
                logger.info(
                    f"Successfully imported focus group record: {statement['id']}"
                )
...
async def import_focus_group_file(
    file: UploadFile = File(...), cohort_name: str = Form(default="focus_group_2024")
):
    ...
    try:
        ...
        request = FocusGroupImportRequest(csv_data=csv_data, cohort_name=cohort_name)
