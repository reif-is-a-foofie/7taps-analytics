...
class OrchestratorTracker:
    ...
    def get_active_agents(self) -> List[Dict[str, Any]]:
        ...
        for module_name, contract in self.contracts.items():
            ...
            if status in ["assigned", "in_progress"]:
                ...
                progress = contract.get("task_tracking", {}).get(
                ...
                active_agents.append(
                    {
                        ...
                        "assigned_at": contract.get("task_tracking", {}).get(
                            "assigned_at"
                        ),
                    }
                )
    ...
    def get_test_results(self) -> Dict[str, Any]:
        ...
        for module_name, contract in self.contracts.items():
            ...
            test_results[module_name] = {
                ...
                "test_file": test_file,
                "test_status": (
                    "pending" if status == "pending" else "ready_for_validation"
                ),
    ...
    def _check_spec_gaming_risk(
        self, module_name: str, contract: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        self_marked_completed = (
            contract.get("status") == "completed" and agent != "orchestrator_agent"
        )
    ...
    def log_database_call(
        self, call_type: str, module: str, details: Dict[str, Any]
    ) -> None:
    ...
    def log_database_call_and_test_result(
        self, call_type: str, module: str, test_result: Dict[str, Any]
    ) -> None:
    ...
    def integrate_test_result_with_database_calls(
        self, test_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Integrate test result with database call tracking."""
        return {
            **test_data,
            "database_calls_related": self._get_related_database_calls(
    ...
    def log_mcp_call_and_test_result(
        self, call_type: str, module: str, test_result: Dict[str, Any]
    ) -> None:
    ...
    def integrate_test_result(self, test_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        integrated_result = {
            "timestamp": datetime.utcnow().isoformat(),
            "test_data": test_data,
            "mcp_calls_related": self._get_related_mcp_calls(
                test_data.get("module", "")
            ),
            "status": "integrated",
        }
...
def log_mcp_call_and_test_result(
    call_type: str, module: str, test_result: Dict[str, Any]
) -> None:
    """Log MCP call with associated test result."""
    return orchestrator_tracker.log_mcp_call_and_test_result(
        call_type, module, test_result
    )
...
async def get_progress() -> Dict[str, Any]:
    ...
    try:
        ...
        completed_modules = sum(
            1 for c in contracts.values() if c.get("status") == "completed"
        )
        overall_progress = (
...
async def get_test_report() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get test report: {str(e)}"
        )
...
async def get_active_agents() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get active agents: {str(e)}"
        )
...
async def get_real_world_testing_status() -> Dict[str, Any]:
    ...
    try:
        ...
        for module_name, result in test_results.items():
            if result.get("requires_real_world_testing", False):
                real_world_modules[module_name] = {
                    ...
                    "ready_for_real_world_test": result["status"]
        ...
        return {
            ...
            "ready_for_real_world_test": sum(
                1 for m in real_world_modules.values() if m["ready_for_real_world_test"]
            ),
        }

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to get real-world testing status: {str(e)}"
        )
...
async def get_anti_spec_gaming_status() -> Dict[str, Any]:
    ...
    try:
                ...
                high_risk_modules.append(
                    {
                        ...
                        "requires_independent_testing": spec_gaming_risk.get(
                            "requires_independent_testing", False
                        ),
                        "requires_orchestrator_review": spec_gaming_risk.get(
                            "requires_orchestrator_review", False
                        ),
                    }
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get anti-spec-gaming status: {str(e)}"
        )
...
async def update_contract(request: ContractUpdateRequest) -> Dict[str, Any]:
    ...
    try:
        if request.module not in orchestrator_tracker.contracts:
            raise HTTPException(
                status_code=404, detail=f"Module {request.module} not found"
            )
