...
async def readiness_probe() -> Dict[str, Any]:
    ...
    try:
        ...
        is_ready = (
            redis_health["status"] == "healthy" and db_health["status"] == "healthy"
        )
...
async def comprehensive_status() -> Dict[str, Any]:
    ...
    try:
        with performance_tracker.track_operation("comprehensive_status_check"):
            ...
            # CPU alert
            if (
                "cpu_percent" in system_resources
                and system_resources["cpu_percent"] > 80
            ):
                alerts.append(
                    {
                        "level": "warning",
            ...
            # Memory alert
            if (
                "memory" in system_resources
                and system_resources["memory"]["used_percent"] > 85
            ):
                alerts.append(
                    {
                        "level": "warning",
            ...
            # Disk alert
            if (
                "disk" in system_resources
                and system_resources["disk"]["used_percent"] > 90
            ):
                alerts.append(
                    {
                        "level": "critical",
