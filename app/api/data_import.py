    ...
    try:
        ...
        for _, row in df.iterrows():
            poll_record = {
                ...
                "metadata": (
                    json.loads(row["metadata"]) if pd.notna(row["metadata"]) else {}
                ),
            }
...
def convert_poll_to_xapi_statement(poll_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    statement = {
        ...
        "actor": {
            ...
            "account": {"name": poll_data["user_id"], "homePage": "https://7taps.com"},
        },
        "verb": {
        ...
        "object": {
            ...
            "objectType": "Activity",
            "definition": {
                "name": {"en-US": poll_data["question"]},
                "description": {"en-US": f"{poll_data['card_type']} question"},
        ...
        "result": {
            ...
            "score": {
                "scaled": (
                    poll_data["score"] / 100.0 if poll_data["score"] is not None else 0
                ),
...
def convert_audio_to_xapi_statement(audio_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    statement = {
        ...
        "actor": {
            ...
            "account": {"name": audio_data["user_id"], "homePage": "https://7taps.com"},
        },
        "verb": {
        ...
        "object": {
            ...
            "objectType": "Activity",
            "definition": {
                "name": {"en-US": "Audio Recording"},
                "description": {"en-US": "Voice response recording"},
        ...
        if polls_file:
            try:
                ...
                for poll_data in polls_data:
                    try:
                        statement = convert_poll_to_xapi_statement(poll_data)
                        await get_normalizer().process_statement_normalization(
                            statement
                        )
                        polls_imported += 1
                        total_imported += 1
...
async def test_import_endpoint():
    ...
    return {
        "status": "data_import_router_loaded",
        "message": "Data import router is working",
    }
