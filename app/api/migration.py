...
async def trigger_migration(
    ...
    try:
        ...
        if not await migrator.validate_schema():
            raise HTTPException(
                status_code=500,
                detail="Schema validation failed. Required tables missing.",
            )
        # Get current stats
...
async def get_migration_status():
    ...
    try:
        ...
        if migration_state["total_statements"] > 0:
            progress = (
                migration_state["migrated_count"] / migration_state["total_statements"]
            ) * 100.0
...
async def migration_health_check():
    ...
    try:
        ...
        return {"status": "unhealthy", "message": f"Migration system error: {str(e)}"}
