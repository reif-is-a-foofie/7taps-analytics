...
class ProductionMonitor:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_data_metrics = self.data_metrics_history[-60:] if len(self.data_metrics_history) >= 60 else self.data_metrics_history
            
            avg_processing_rate = sum(m.processing_rate for m in recent_data_metrics) / len(recent_data_metrics) if recent_data_metrics else 0
            avg_error_rate = sum(m.error_rate for m in recent_data_metrics) / len(recent_data_metrics) if recent_data_metrics else 0
            ...
            return {
                "system_performance": {
                    ...
                    "connection_stability": {
                        "database": all(m.database_connected for m in recent_metrics[-10:]) if recent_metrics else False,
                        "redis": all(m.redis_connected for m in recent_metrics[-10:]) if recent_metrics else False
                    }
                },
                "data_performance": {
                    ...
                    "data_integrity": {
                        "statements_flat": self.data_metrics_history[-1].statements_flat_count if self.data_metrics_history else 0,
                        "statements_normalized": self.data_metrics_history[-1].statements_normalized_count if self.data_metrics_history else 0,
                        "normalization_ratio": round(
                            (self.data_metrics_history[-1].statements_normalized_count / 
                             max(self.data_metrics_history[-1].statements_flat_count, 1)) * 100, 2
                        ) if self.data_metrics_history else 0
                    }
                },
                "alerts_summary": {
    ...
    def get_metrics_history(self, hours: int = 24) -> Dict[str, Any]:
        ...
        data_metrics = [
            asdict(m) for m in self.data_metrics_history 
            if m.timestamp >= cutoff_time
        ]
