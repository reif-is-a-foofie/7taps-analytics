...
class LearningLockerSync:
    ...
    def __init__(self):
        ...
        self.learninglocker_url = os.getenv(
            "LEARNINGLOCKER_URL", "http://localhost:8080"
        )
        self.learninglocker_username = os.getenv(
            "LEARNINGLOCKER_USERNAME", "admin@7taps.com"
        )
    ...
    async def get_learninglocker_auth(self) -> str:
        ...
        try:
            ...
            response = await self._http_client.post(
                f"{self.learninglocker_url}/auth/login", json=login_data
            )
    ...
    async def sync_statement_to_learninglocker(self, statement: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                logger.info(
                    f"Successfully synced statement {statement.get('id', 'unknown')} to Learning Locker"
                )
                return True
            else:
                logger.error(
                    f"Failed to sync statement: {response.status_code} - {response.text}"
                )
                return False
    ...
    async def sync_redis_to_learninglocker(
        self, max_statements: int = 10
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                self.redis_client.xgroup_create(
                    stream_name, sync_group, id="0", mkstream=True
                )
            ...
            for stream, message_list in messages:
                for message_id, fields in message_list:
                    try:
                        ...
                        success = await self.sync_statement_to_learninglocker(
                            statement_data
                        )
    ...
    async def get_sync_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "redis_url": (
                self.redis_url.split("@")[-1] if self.redis_url else "not_configured"
            ),
        }
